...
(async function(){
  ...
  // produce the same base slug as makeHashId but without uniqueness suffix
  function makeSimpleSlug(p, idx){
    let base = '';
    const tryFields = ['username','handle','user','screen_name','profile_name','id'];
    for(const k of tryFields){ if(p[k] && String(p[k]).trim()){ base = String(p[k]).trim(); break; } }
    if(!base){ const url = p.url || p.profile_url || p.profile || ''; if(url && String(url).trim()){ try{ const u = new URL(String(url)); const segs = u.pathname.split('/').filter(Boolean); if(segs.length) base = segs[segs.length-1]; else base = u.hostname.replace(/^www\./,''); }catch(e){ base = String(url); } } }
    ...
    base = String(base);
    base = base.replace(/\.[a-z0-9]{1,5}$/i,'').replace(/[^a-z0-9-_]+/ig,'-').replace(/(^-|-$)/g,'');
    let slug = base.toLowerCase(); if(!slug) slug = 'profile';
    return slug;
  }
  ...
  function doSearch(){
    ...
    if(cur){
      ...
      let target = profiles.find(p=> p._hashId === dec);
      if(!target){ // fallback: match simple slug variants (same logic groups.js uses)
        for(let i=0;i<profiles.length;i++){ try{ if(makeSimpleSlug(profiles[i], i) === dec){ target = profiles[i]; break; } }catch(e){} }
      }
      if(target) return showDetail(target);
    }
  ...
  (function openFromHash(){
    ...
    if(cur){
      try{
        const dec = decodeURIComponent(cur);
        let target = profiles.find(p=> p._hashId === dec);
        if(!target){ // fallback: try simple slug match (covers slugs generated by groups.js)
          for(let i=0;i<profiles.length;i++){ try{ if(makeSimpleSlug(profiles[i], i) === dec){ target = profiles[i]; break; } }catch(e){} }
        }
